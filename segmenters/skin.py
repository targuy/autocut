...
class SkinSegmenter:
    ...
    def detect(self, image: np.ndarray, person_mask: np.ndarray = None) -> Tuple[np.ndarray, float]:
        """
        ...
        if person_mask is not None:
            combined_mask &= (person_mask > 0)
        skin_area = np.sum(combined_mask)
        skin_pct = 100.0 * skin_area / (np.sum(person_mask) if person_mask is not None else total_area)
        return combined_mask, skin_pct
