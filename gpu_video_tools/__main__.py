...
def cutlist(ctx, input, output, cuts, timecode_overlay):
    ...
    try:
        ...
        # Read cuts CSV
        segments = []
        with open(cuts, 'r') as f:
            reader = csv.DictReader(f)
            for row in reader:
                start = float(row.get('start_time', row.get('start', 0)))
                end = float(row.get('end_time', row.get('end', 0)))
                segments.append((start, end))
        
        if not segments:
            print_error_panel("No segments found in cuts CSV", console)
            sys.exit(1)
        
        console.log(f"Found {len(segments)} segments to concatenate")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for i, (start, end) in enumerate(segments):
                ...
                console.log(f"Extracting segment {i+1}/{len(segments)}: {start:.3f}s - {end:.3f}s")
                ...
                if result.returncode != 0:
                    print_error_panel(f"Failed to extract segment {i+1}", console)
                    console.log(f"[red]{result.stderr.decode()}[/red]")
                    sys.exit(1)
            ...
            # Add timecode overlay if requested
            if timecode_overlay:
                cmd = [
                    'ffmpeg', '-y',
                    '-f', 'concat',
            ...
            result = subprocess.run(cmd, capture_output=True)
            if result.returncode != 0:
                print_error_panel("Failed to concatenate segments", console)
                console.log(f"[red]{result.stderr.decode()}[/red]")
                sys.exit(1)
        
        print_success_panel(f"Created cutlist video: {output}", console)
        console.log(f"Concatenated {len(segments)} segments")
        
    except Exception as e:
        print_error_panel(f"Cutlist failed: {e}", console)
        sys.exit(1)
