...
def get_driver_version(device: Optional[ResolvedDevice]) -> str:
    """Get driver version for the device.
    
    Args:
        device: Resolved device
    
    Returns:
        Driver version string or empty string if unavailable
    """
    if not device:
        return ''
    ...
    try:
        if device.vendor == 'nvidia':
            ...
            # Try to get AMD driver version on Windows
            try:
                import platform
                if platform.system() == 'Windows':
                    ...
                    for video_controller in w.Win32_VideoController():
                        if 'AMD' in video_controller.Name or 'Radeon' in video_controller.Name:
                            return video_controller.DriverVersion or ''
            except Exception:
                return ''
    except Exception:
        pass
...
class BenchmarkLogger:
    ...
    def log_result(
        ...
        # Get driver version
        driver = get_driver_version(device)
